 ...
 * cwd, but an embedding host (SDK, pi-web) creates sessions whose workspace is
 * not the directory the process started in — and can run several such sessions
 * concurrently, where any process-wide registry is last-writer-wins.
 *
 * Ported from pr-223-merge onto the v0.19 architecture (per-session
 * AgentTypeState in the activation closure; registry threaded through
 * SpawnOptions; Agent tool re-registration on cwd adoption).
 *
 * Three pins:
...
function makePi() {
  ...
  const pi = {
    ...
    registerFlag: vi.fn(),
    registerCommand: vi.fn((c: any) => lifecycle.set(`cmd:${c.name}`, c)),
...
function makeCtx(cwd: string) {
  return {
    ...
    sessionManager: { getSessionId: vi.fn(() => "s1"), getBranch: vi.fn(() => []), getCwd: vi.fn(() => cwd) },
...
describe("per-session config discovery through the real extension (#206)", () => {
  ...
  it("adopts a diverging session cwd: settings, agents, and the Agent tool schema rebuild from it", async () => {
    ...
    expect(runAgent).toHaveBeenCalledWith(expect.anything(), "beta", "go", expect.objectContaining({ registry: expect.any(Map) }));
  ...
  it("resets provisional-cwd policy at adoption: keys the session's tree omits return to defaults", async () => {
    ...
    expect(runAgent).toHaveBeenCalledWith(expect.anything(), "general-purpose", "go", expect.objectContaining({ registry: expect.any(Map) }));
  });
  ...
  it("two concurrent sessions resolve their own agents — no last-writer-wins", async () => {
    ...
    expect(runAgent).toHaveBeenCalledWith(expect.anything(), "alpha", "go", expect.objectContaining({ registry: expect.any(Map) }));
  });
});
