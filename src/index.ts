...
export default function (pi: ExtensionAPI) {
  ...
  const spawnResolved = (piRef: any, ctxRef: any, type: string, prompt: string, options: any) => {
    ...
    // definitions the spawn resolves against, so callers get the session's
    // own — a forged one would smuggle in arbitrary definitions.
    const spawnOptions = {
      ...options,
      registry: agentTypes.registry(),
      buildRegistryFor,
    };
    return manager.spawn(piRef, ctxRef, dispatch.type, prompt, spawnOptions);
  };
  ...
  // Functions (not consts) so the #206 session-cwd adoption re-render reflects
  // the session's own settings, same trade as scheduleGuideline.
...
**Never delegate understanding.** Don't write "based on your findings, fix the bug" or "based on the research, implement it." Those phrases push synthesis onto the agent instead of doing it yourself. Write prompts that prove you understood: include file paths, line numbers, what specifically to change.`;
  ...
  const renderToolDescriptionTemplate = (template: string): string => {
    // Normalize Windows line endings: template files written on Windows carry
    // CRLF, and both the byte-identical guard below and the model expect LF.
    ...
    return normalized.replace(/\{\{(\w+)\}\}/g, (raw, name: string) => {
   ...
   *
   * Held in a closure variable: the mention clone reuses this exact definition
   * (mention-clone.ts), so the agent it starts is an ordinary top-level spawn
   * instead of a second implementation that has to be kept in step with this
   * one.
   */
  let agentTool: ReturnType<typeof defineTool>;
  ...
  async function ejectAgent(ctx: ExtensionCommandContext, name: string, cfg: AgentConfig) {
    ...
    const targetDir = location.startsWith("Project") ? projectAgentsDir(sessionCwd) : personalAgentsDir();
